    ...
    pub(crate) fn add_report(&self, report: GameReport) {
        match self.inner.lock() {
            ...
            Err(error) => {
                ...
                tracing::error!(target: Log::Slippi, ?error, "Unable to lock queue, dropping report");
            },
        }
    }
    ...
    pub fn report_abandonment(&self, uid: String, play_key: String, match_id: String) {
        ...
        match res {
            Ok(value) if value == "true" => {
                tracing::info!(target: Log::Slippi, "Successfully executed abandonment request")
            },
            Ok(value) => tracing::error!(target: Log::Slippi, ?value, "Error executing abandonment request",),
            Err(error) => tracing::error!(target: Log::Slippi, ?error, "Error executing abandonment request"),
        }
    }
}
    ...
    loop {
        ...
        match receiver.recv() {
            ...
            Ok(CompletionEvent::Shutdown) => {
                tracing::info!(target: Log::Slippi, "Completion thread winding down");
                break;
            },
...
pub fn report_completion(http_client: &ureq::Agent, uid: String, match_id: String, play_key: String, end_mode: u8) {
    ...
    match res {
        Ok(value) if value == "true" => {
            tracing::info!(target: Log::Slippi, "Successfully executed completion request")
        },
        Ok(value) => tracing::error!(target: Log::Slippi, ?value, "Error executing completion request",),
        Err(error) => tracing::error!(target: Log::Slippi, ?error, "Error executing completion request"),
    }
}
    ...
    loop {
        ...
        match receiver.recv() {
            ...
            Ok(ProcessingEvent::Shutdown) => {
                tracing::info!(target: Log::Slippi, "Processing thread winding down");
...
fn process_reports(queue: &GameReporterQueue, event: ProcessingEvent) {
    let Ok(iso_hash) = queue.iso_hash.lock() else {
        tracing::warn!(target: Log::Slippi, "No ISO_HASH available");
        return;
    };

    let Ok(mut report_queue) = queue.inner.lock() else {
        tracing::warn!(target: Log::Slippi, "Reporter Queue is dead");
        return;
    };
    ...
    while !report_queue.is_empty() {
        ...
        match try_send_next_report(&mut *report_queue, event, &queue.http_client, &iso_hash) {
            Ok(upload_url) => {
                ...
                tracing::info!(target: Log::Slippi, "Successfully sent report, popping from queue");
            ...
            Err(error) => {
                ...
                if error.is_last_attempt {
                    tracing::error!(target: Log::Slippi, "Hit max retry limit, dropping report");
...
fn try_upload_replay_data(data: Vec<u8>, upload_url: String, http_client: &ureq::Agent) {
    ...
    let res_size = match compress_to_gzip(&contents, &mut gzipped_data) {
        ...
        Err(error) => {
            tracing::error!(target: Log::Slippi, ?error, "Failed to compress replay");
            return;
        },
    };
    ...
    if let Err(error) = response {
        tracing::error!(target: Log::Slippi, ?error, "Failed to upload replay data",);
    }
}
