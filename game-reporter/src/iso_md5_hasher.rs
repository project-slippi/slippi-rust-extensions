...
pub fn run(iso_hash: Arc<Mutex<String>>, iso_path: String) {
    let digest = match File::open(&iso_path) {
        Ok(file) => match chksum::<MD5, _>(file) {
            ...
            Err(error) => {
                tracing::error!(target: Log::Slippi, ?error, "Unable to produce ISO MD5 Hash");
        ...
        Err(error) => {
            tracing::error!(target: Log::Slippi, ?error, "Unable to open ISO for MD5 hashing");
    ...
    if !KNOWN_DESYNC_ISOS.contains(&hash.as_str()) {
        tracing::info!(target: Log::Slippi, iso_md5_hash = ?hash);
    } else {
    ...
    match iso_hash.lock() {
        ...
        Err(error) => {
            tracing::error!(target: Log::Slippi, ?error, "Unable to lock iso_hash");
        },
    };
}
